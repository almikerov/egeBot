...
async def _run_blocking_call(func, *args, **kwargs):
    """Асинхронно выполняет блокирующую функцию в отдельном потоке."""
    loop = asyncio.get_running_loop()
    # partial используется для передачи функции с ее аргументами в executor
...
async def get_sheet_titles() -> list:
    ...
    try:
        # --- ИЗМЕНЕНИЕ ЗДЕСЬ ---
        # Выполняем блокирующий вызов .execute() в отдельном потоке
        sheet_metadata = await _run_blocking_call(
...
async def get_task_from_sheet(sheet_title: str) -> tuple:
    ...
    try:
        # --- ИЗМЕНЕНИЕ ЗДЕСЬ ---
        request = service.spreadsheets().values().get(spreadsheetId=SPREADSHEET_ID, range=f"'{sheet_title}'!A1:E")
        result = await _run_blocking_call(request.execute)
        # --- КОНЕЦ ИЗМЕНЕНИЯ ---
        ...
        tasks = values[2:]
        if not tasks: return prompt, None

        valid_tasks = [row for row in tasks if len(row) >= 3 and row[0] and row[2]]
        if not valid_tasks: return prompt, None
...
async def get_task_by_id(task_id: str) -> tuple:
    ...
    if not sheet_titles: return None, None

    for title in sheet_titles:
        try:
            # --- ИЗМЕНЕНИЕ ЗДЕСЬ ---
            request = service.spreadsheets().values().get(spreadsheetId=SPREADSHEET_ID, range=f"'{title}'!A1:E")
            result = await _run_blocking_call(request.execute)
            # --- КОНЕЦ ИЗМЕНЕНИЯ ---
